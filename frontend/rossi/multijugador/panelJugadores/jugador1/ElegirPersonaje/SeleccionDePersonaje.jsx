    ...
    return (
        <>
            <div id='SeleccionDePersonaje' className="flex flex-col justify-around items-center h-screen text-black" style={{ backgroundImage: `url(${Background})`, backgroundSize: "cover", backgroundPosition: "center" }}>
                ...
                {!TerminarSeleccionJugador && !Datos && <>
                    <div className={`${Fondo ? "bg-orange-500" : "bg-blue-500"} flex flex-col justify-center items-center gap-4 bg-white rounded-xl p-4 w-1/2`}>
                        ...
                        <button className={`${Fondo ? "bg-orange-500" : "bg-blue-500"} rounded-xl p-4 text-white`} onClick={() => savePersonaje(personaje)}>Terminar Turno</button>
                    </div>

                </>}
                {Datos && <>
                    <div className={`${Fondo ? "flex flex-col justify-center items-center gap-4 rounded-xl p-4 w-1/2 bg-gradient-to-r from-orange-500 to-orange-800 border-4 border-black" : "flex flex-col justify-center items-center gap-4 rounded-xl p-4 w-1/2 bg-gradient-to-r from-blue-500 to-blue-800 border-4 border-black"}`}  >
                        <h3>Las estadisticas de {Datos.Datos.jugador.name} son:</h3>
                        <p>El personaje que eligio fue {Datos.Datos.jugador.personaje}</p>
                        <p>El puntaje es de {Datos.Datos.puntaje}</p>
                        <p>{(Datos.Datos.suma === 0) ? "No hay Suma" : "La suma es de " + Datos.Datos.suma}</p>
                        <p>{(Datos.Datos.resta === 0) ? "No hay Resta" : "La resta es de " + Datos.Datos.resta}</p>
                        <p>{(Datos.Datos.multiplicador === 1) ? "No hay Multiplicador" : "El multiplicador es de " + Datos.Datos.multiplicador}</p>
