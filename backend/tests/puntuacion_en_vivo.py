...
def puntuacion_en_vivo(camara):
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q') or len(puntaje) > 0:
            break
...
while True:
    time.sleep(1)
    puntaje = puntuacion_en_vivo(camara)
    url = "http://localhost:5000/recibir"
    dato = {"puntaje": sum(puntaje), "ids": circulos_ids}  # cualquier dato que quieras mandar
