...
def detectar_formas_puntuacion(x0=84, y0=78, ancho_total=468, alto_total=350, columnas=7, filas=5):
        ...
        try:
            while cap.isOpened():
                        ...
                        if circles is not None:
                            incio_de_deteccion = time.time()
                            time.sleep(0.1)
                            circles = np.uint16(np.around(circles))
                            for circle in circles[0, :]:
                                cx, cy, r = circle
                                cv2.circle(resized_image, (cx, cy), r, (0, 255, 0), 5)
